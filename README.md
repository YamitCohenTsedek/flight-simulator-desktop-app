...
# In this milestone we built a Gui interface (WPF app) which allows us to control our aircraft. We set up 2 TCP communication channels built into our code as follows:* Commands Channel - In this channel, we will connect as a customer to the flight simulator which will serve as a server. In this channel we will pass basic set commands to the flight simulator.
# • Channel Info - In this channel the simulator will connect as a customer to us, and send us the data about the plane the data will be defined in the generic_small.xml file. Thus, we have created a server that will listen to the aircraft data in particular Lon, Lat.
